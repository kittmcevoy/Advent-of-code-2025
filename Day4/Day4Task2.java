...
public class Day4Task2 {
    ...
    int solvePromlem(String[] grid) {
        ...
        // result matrix (copy of original)
        char[][] out = new char[rows][cols];
        for (int r = 0; r < rows; r++) {
            System.arraycopy(matrix[r], 0, out[r], 0, cols);
        }

        int[] dr = { -1, -1, -1, 0, 0, 1, 1, 1 };
        int[] dc = { -1, 0, 1, -1, 1, -1, 0, 1 };

        int previousAccessibleCount = 1;
        int accessibleCount = 0;

        while (previousAccessibleCount > 0) {
            previousAccessibleCount = 0;
            for (int r = 0; r < rows; r++) {
